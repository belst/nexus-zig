...
#else
#if !defined(IMGUI_USE_STB_SPRINTF)&& defined(__GNUC__)&& defined(__MINGW32__)
#define IM_FMTARGS(FMT)             __attribute__((format(gnu_printf, FMT, FMT+1)))
#define IM_FMTLIST(FMT)             __attribute__((format(gnu_printf, FMT, 0)))
#else
#define IM_FMTARGS(FMT)
#define IM_FMTLIST(FMT)
#endif // #if !defined(IMGUI_USE_STB_SPRINTF)&& defined(__GNUC__)&& defined(__MINGW32__)
...
#endif// ImTextureID [configurable type: override in imconfig.h with '#define ImTextureID xxx']
...
#else
#if (defined(__clang__)|| defined(__GNUC__))&&(__cplusplus<201100)
#include <stdint.h>
...
#endif // #if (defined(__clang__)|| defined(__GNUC__))&&(__cplusplus<201100)
...
CIMGUI_API void ImGui_SetWindowFontScale(float scale);                                                              // set font scale. Adjust IO.FontGlobalScale if you want to scale all windows. This is an old API! For correct scaling, prefer to reload font + rebuild ImFontAtlas + call style.ScaleAllSizes().
...
CIMGUI_API ImVec2 ImGui_GetWindowContentRegionMin(void);    // content boundaries min (roughly (0,0)-Scroll), in window coordinates
CIMGUI_API ImVec2 ImGui_GetWindowContentRegionMax(void);    // content boundaries max (roughly (0,0)+Size-Scroll) where Size can be override with SetNextWindowContentSize(), in window coordinates
...
CIMGUI_API void ImGui_PushAllowKeyboardFocus(bool allow_keyboard_focus);  // allow focusing using TAB/Shift-TAB, enabled by default but you can disable it for certain widgets
...
CIMGUI_API void  ImGui_PushItemWidth(float item_width);                       // push width of items for common large "item+label" widgets. >0.0f: width in pixels, <0.0f align xx pixels to the right of window (so -FLT_MIN always align width to the right side). 0.0f = default to ~2/3 of windows width,
...
CIMGUI_API ImVec2 ImGui_GetCursorScreenPos(void);                 // cursor position in absolute screen coordinates [0..io.DisplaySize] (useful to work with ImDrawList API)
CIMGUI_API void   ImGui_SetCursorScreenPos(ImVec2 pos);           // cursor position in absolute screen coordinates [0..io.DisplaySize]
...
CIMGUI_API bool ImGui_ImageButton(ImTextureID user_texture_id, ImVec2 size);                                                       // Implied uv0 = ImVec2(0, 0), uv1 = ImVec2(1, 1), frame_padding = -1, bg_col = ImVec4(0, 0, 0, 0), tint_col = ImVec4(1, 1, 1, 1)
CIMGUI_API bool ImGui_ImageButtonEx(ImTextureID user_texture_id, ImVec2 size, ImVec2 uv0 /* = ImVec2(0, 0) */, ImVec2 uv1 /* = ImVec2(1, 1) */, int frame_padding /* = -1 */, ImVec4 bg_col /* = ImVec4(0, 0, 0, 0) */, ImVec4 tint_col /* = ImVec4(1, 1, 1, 1) */); // <0 frame_padding uses default frame padding settings. 0 for no padding
...
CIMGUI_API bool ImGui_DragScalar(const char* label, ImGuiDataType data_type, void* p_data, float v_speed);                   // Implied p_min = NULL, p_max = NULL, format = NULL, flags = 0
CIMGUI_API bool ImGui_DragScalarEx(const char* label, ImGuiDataType data_type, void* p_data, float v_speed, const void* p_min /* = NULL */, const void* p_max /* = NULL */, const char* format /* = NULL */, ImGuiSliderFlags flags /* = 0 */);
CIMGUI_API bool ImGui_DragScalarN(const char* label, ImGuiDataType data_type, void* p_data, int components, float v_speed);  // Implied p_min = NULL, p_max = NULL, format = NULL, flags = 0
CIMGUI_API bool ImGui_DragScalarNEx(const char* label, ImGuiDataType data_type, void* p_data, int components, float v_speed, const void* p_min /* = NULL */, const void* p_max /* = NULL */, const char* format /* = NULL */, ImGuiSliderFlags flags /* = 0 */);
...
CIMGUI_API bool ImGui_ListBoxHeader(const char* label, ImVec2 size /* = ImVec2(0, 0) */);                    // use if you want to reimplement ListBox() will custom data or interactions. if the function return true, you can output elements then call ListBoxFooter() afterwards.
CIMGUI_API bool ImGui_ListBoxHeaderInt(const char* label, int items_count, int height_in_items /* = -1 */);  // "
CIMGUI_API void ImGui_ListBoxFooter(void);                                                                   // terminate the scrolling region. only call ListBoxFooter() if ListBoxHeader() returned true!
...
CIMGUI_API bool ImGui_MenuItemEx(const char* label, const char* shortcut /* = NULL */, bool selected /* = false */, bool enabled /* = true */); // return true when activated. shortcuts are displayed for convenience but not processed by ImGui at the moment
...
CIMGUI_API void ImGui_OpenPopupOnItemClick(const char* str_id /* = NULL */, ImGuiPopupFlags popup_flags /* = 1 */);       // helper to open popup when clicked on last item. return true when just opened. (note: actually triggers on the mouse _released_ event to be consistent with popup behaviors)
...
CIMGUI_API void                  ImGui_TableSetupColumnEx(const char* label, ImGuiTableColumnFlags flags /* = 0 */, float init_width_or_weight /* = 0.0f */, ImU32 user_id /* = 0 */);
...
CIMGUI_API ImGuiTableSortSpecs*  ImGui_TableGetSortSpecs(void);                                                                    // get latest sort specs for the table (NULL if not sorting).
...
CIMGUI_API bool                ImGui_BeginDragDropSource(ImGuiDragDropFlags flags /* = 0 */);                      // call when the current item is active. If this return true, you can call SetDragDropPayload() + EndDragDropSource()
CIMGUI_API bool                ImGui_SetDragDropPayload(const char* type, const void* data, size_t sz, ImGuiCond cond /* = 0 */); // type is a user defined string of maximum 32 characters. Strings starting with '_' are reserved for dear imgui internal types. Data is copied and held by imgui.
...
CIMGUI_API bool   ImGui_IsItemClickedEx(ImGuiMouseButton mouse_button /* = 0 */);  // is the last item clicked? (e.g. button/node just clicked on) == IsMouseClicked(mouse_button) && IsItemHovered()
...
CIMGUI_API ImDrawList*           ImGui_GetBackgroundDrawList(void);                                                 // this draw list will be the first rendering one. Useful to quickly draw shapes/text behind dear imgui contents.
...
CIMGUI_API void                  ImGui_CalcListClipping(int items_count, float items_height, int* out_items_display_start, int* out_items_display_end); // calculate coarse clipping for large list of evenly sized items. Prefer using the ImGuiListClipper higher-level helper if you can.
...
CIMGUI_API int  ImGui_GetKeyIndex(ImGuiKey imgui_key);                                        // map ImGuiKey_* values into user's key index. == io.KeyMap[key]
CIMGUI_API bool ImGui_IsKeyDown(int user_key_index);                                          // is key being held. == io.KeysDown[user_key_index].
CIMGUI_API bool ImGui_IsKeyPressed(int user_key_index);                                       // Implied repeat = true
CIMGUI_API bool ImGui_IsKeyPressedEx(int user_key_index, bool repeat /* = true */);           // was key pressed (went from !Down to Down)? if repeat=true, uses io.KeyRepeatDelay / KeyRepeatRate
CIMGUI_API bool ImGui_IsKeyReleased(int user_key_index);                                      // was key released (went from Down to !Down)?
CIMGUI_API int  ImGui_GetKeyPressedAmount(int key_index, float repeat_delay, float rate);     // uses provided repeat rate/delay. return a count, most often 0 or 1 but might be >1 if RepeatRate is small enough that DeltaTime > RepeatRate
CIMGUI_API void ImGui_CaptureKeyboardFromApp(bool want_capture_keyboard_value /* = true */);  // attention: misleading name! manually override io.WantCaptureKeyboard flag next frame (said flag is entirely left for your application to handle). e.g. force capture keyboard when your widget is being hovered. This is equivalent to setting "io.WantCaptureKeyboard = want_capture_keyboard_value"; after the next NewFrame() call.
...
CIMGUI_API bool             ImGui_IsMouseClickedEx(ImGuiMouseButton button, bool repeat /* = false */);          // did mouse button clicked? (went from !Down to Down)
CIMGUI_API bool             ImGui_IsMouseReleased(ImGuiMouseButton button);                                      // did mouse button released? (went from Down to !Down)
CIMGUI_API bool             ImGui_IsMouseDoubleClicked(ImGuiMouseButton button);                                 // did mouse button double-clicked? (note that a double-click will also report IsMouseClicked() == true)
...
CIMGUI_API bool             ImGui_IsAnyMouseDown(void);                                                          // is any mouse button held?
...
CIMGUI_API void             ImGui_CaptureMouseFromApp(bool want_capture_mouse_value /* = true */);               // attention: misleading name! manually override io.WantCaptureMouse flag next frame (said flag is entirely left for your application to handle). This is equivalent to setting "io.WantCaptureMouse = want_capture_mouse_value;" after the next NewFrame() call.
...
CIMGUI_API void  ImGui_SetAllocatorFunctions(void* (*alloc_func)(size_t sz, void* user_data), void (*free_func)(void* ptr, void* user_data), void* user_data /* = NULL */);
...
struct ImVector_ImGuiStoragePair_t { int Size; int Capacity; ImGuiStorage_ImGuiStoragePair* Data; };  // Instantiation of ImVector<ImGuiStoragePair>
...
// Functions
CIMGUI_API void ImGuiIO_AddInputCharacter(ImGuiIO* self, unsigned int c);        // Queue new character input
CIMGUI_API void ImGuiIO_AddInputCharacterUTF16(ImGuiIO* self, ImWchar16 c);      // Queue new character input from an UTF-16 character, it can be a surrogate
CIMGUI_API void ImGuiIO_AddInputCharactersUTF8(ImGuiIO* self, const char* str);  // Queue new characters input from an UTF-8 string
...
CIMGUI_API bool        ImGui_DragScalarFloat(const char* label, ImGuiDataType data_type, void* p_data, float v_speed, const void* p_min, const void* p_max, const char* format, float power);
CIMGUI_API bool        ImGui_DragScalarNFloat(const char* label, ImGuiDataType data_type, void* p_data, int components, float v_speed, const void* p_min, const void* p_max, const char* format, float power);
CIMGUI_API bool        ImGui_DragFloatFloat(const char* label, float* v, float v_speed, float v_min, float v_max, const char* format, float power);
CIMGUI_API bool        ImGui_DragFloat2Float(const char* label, float v[2], float v_speed, float v_min, float v_max, const char* format, float power);
CIMGUI_API bool        ImGui_DragFloat3Float(const char* label, float v[3], float v_speed, float v_min, float v_max, const char* format, float power);
CIMGUI_API bool        ImGui_DragFloat4Float(const char* label, float v[4], float v_speed, float v_min, float v_max, const char* format, float power);
CIMGUI_API bool        ImGui_SliderScalarFloat(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, float power);
CIMGUI_API bool        ImGui_SliderScalarNFloat(const char* label, ImGuiDataType data_type, void* p_data, int components, const void* p_min, const void* p_max, const char* format, float power);
CIMGUI_API bool        ImGui_SliderFloatFloat(const char* label, float* v, float v_min, float v_max, const char* format, float power);
CIMGUI_API bool        ImGui_SliderFloat2Float(const char* label, float v[2], float v_min, float v_max, const char* format, float power);
CIMGUI_API bool        ImGui_SliderFloat3Float(const char* label, float v[3], float v_min, float v_max, const char* format, float power);
CIMGUI_API bool        ImGui_SliderFloat4Float(const char* label, float v[4], float v_min, float v_max, const char* format, float power);
// OBSOLETED in 1.77 (from June 2020)
CIMGUI_API bool        ImGui_BeginPopupContextWindowImGuiMouseButton(const char* str_id, ImGuiMouseButton mb, bool over_items);
// OBSOLETED in 1.72 (from April 2019)
CIMGUI_API void        ImGui_TreeAdvanceToLabelPos(void);
// OBSOLETED in 1.71 (from June 2019)
CIMGUI_API void        ImGui_SetNextTreeNodeOpen(bool open, ImGuiCond cond /* = 0 */);
// OBSOLETED in 1.70 (from May 2019)
CIMGUI_API float       ImGui_GetContentRegionAvailWidth(void);
// OBSOLETED in 1.69 (from Mar 2019)
CIMGUI_API ImDrawList* ImGui_GetOverlayDrawList(void);
// OBSOLETED in 1.66 (from Sep 2018)
CIMGUI_API void        ImGui_SetScrollHere(void);                                                                                                       // Implied center_ratio = 0.5f
CIMGUI_API void        ImGui_SetScrollHereEx(float center_ratio /* = 0.5f */);
#endif // #ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
//-----------------------------------------------------------------------------
// [SECTION] Helpers (ImGuiOnceUponAFrame, ImGuiTextFilter, ImGuiTextBuffer, ImGuiStorage, ImGuiListClipper, ImColor)
//-----------------------------------------------------------------------------
...
CIMGUI_API void ImGuiListClipper_Begin(ImGuiListClipper* self, int items_count, float items_height /* = -1.0f */);  // Automatically called by constructor if you passed 'items_count' or by Step() in Step 1.
...
CIMGUI_API void        ImDrawList_AddRect(ImDrawList* self, ImVec2 p_min, ImVec2 p_max, ImU32 col);                                                                 // Implied rounding = 0.0f, rounding_corners = ImDrawCornerFlags_All, thickness = 1.0f
CIMGUI_API void        ImDrawList_AddRectEx(ImDrawList* self, ImVec2 p_min, ImVec2 p_max, ImU32 col, float rounding /* = 0.0f */, ImDrawCornerFlags rounding_corners /* = ImDrawCornerFlags_All */, float thickness /* = 1.0f */); // a: upper-left, b: lower-right (== upper-left + size), rounding_corners_flags: 4 bits corresponding to which corner to round
CIMGUI_API void        ImDrawList_AddRectFilled(ImDrawList* self, ImVec2 p_min, ImVec2 p_max, ImU32 col);                                                           // Implied rounding = 0.0f, rounding_corners = ImDrawCornerFlags_All
CIMGUI_API void        ImDrawList_AddRectFilledEx(ImDrawList* self, ImVec2 p_min, ImVec2 p_max, ImU32 col, float rounding /* = 0.0f */, ImDrawCornerFlags rounding_corners /* = ImDrawCornerFlags_All */); // a: upper-left, b: lower-right (== upper-left + size)
...
CIMGUI_API void        ImDrawList_AddConvexPolyFilled(ImDrawList* self, const ImVec2* points, int num_points, ImU32 col);                                           // Note: Anti-aliased filling requires points to be in clockwise order.
...
CIMGUI_API void        ImDrawList_AddImageRounded(ImDrawList* self, ImTextureID user_texture_id, ImVec2 p_min, ImVec2 p_max, ImVec2 uv_min, ImVec2 uv_max, ImU32 col, float rounding); // Implied rounding_corners = ImDrawCornerFlags_All
CIMGUI_API void        ImDrawList_AddImageRoundedEx(ImDrawList* self, ImTextureID user_texture_id, ImVec2 p_min, ImVec2 p_max, ImVec2 uv_min, ImVec2 uv_max, ImU32 col, float rounding, ImDrawCornerFlags rounding_corners /* = ImDrawCornerFlags_All */);
...
CIMGUI_API void        ImDrawList_PathStroke(ImDrawList* self, ImU32 col, bool closed, float thickness /* = 1.0f */);
CIMGUI_API void        ImDrawList_PathArcTo(ImDrawList* self, ImVec2 center, float radius, float a_min, float a_max, int num_segments /* = 10 */);
...
CIMGUI_API void        ImDrawList_PathRect(ImDrawList* self, ImVec2 rect_min, ImVec2 rect_max, float rounding /* = 0.0f */, ImDrawCornerFlags rounding_corners /* = ImDrawCornerFlags_All */);
// Advanced
...
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
CIMGUI_API void ImDrawList_AddBezierCurve(ImDrawList* self, ImVec2 p1, ImVec2 p2, ImVec2 p3, ImVec2 p4, ImU32 col, float thickness, int num_segments /* = 0 */);
CIMGUI_API void ImDrawList_PathBezierCurveTo(ImDrawList* self, ImVec2 p2, ImVec2 p3, ImVec2 p4, int num_segments /* = 0 */);
#endif // #ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
// [Internal helpers]
